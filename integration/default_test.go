...
func testDefault(t *testing.T, context spec.G, it spec.S) {
	...
	context("when building a simple app with no dependencies", func() {
		...
		it("builds successfully", func() {
			...
			Expect(err).NotTo(HaveOccurred())
			Expect(string(contents)).To(ContainSubstring(`"type":"directory"`))
			Expect(string(contents)).To(ContainSubstring(fmt.Sprintf(`"/layers/%s/targets/bin"`, strings.ReplaceAll(settings.Buildpack.ID, "/", "_"))))
		})
	})
